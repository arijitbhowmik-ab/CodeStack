...
const UpdateCourse = () => {
    ...
    useEffect(()=> {
        const fetchCourseData = async () => {
            try {
                const {data} = await axios.get(`http://localhost:8080/api/v1/course/${id}`, {withCredentials: true})
                console.log("Course fetched successfully: ", data)
                setTitle(data.course.title)
                setDescription(data.course.description)
                setPrice(data.course.price)
                setImagePreview(data.course.images.url)
                setLoading(false)
    ...
    const handleUpdateCourse = async (e) =>{
        ...
        try {
            ...
            console.log(error)
            toast.error(error.response.data.errors || "Failed to update course. Try again.")
        }
    }
  return (
    <div>
