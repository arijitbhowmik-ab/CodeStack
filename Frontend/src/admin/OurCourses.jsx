...
const OurCourses = () => {
    ...
    const adminName = admin.admin.firstName + " " + admin.admin.lastName
    ...
    if (!token) {
        ...
        const handleDeleteCourse = async (id) => {
            try {
                ...
                toast.error(error.response.data.errors || "Error occurred while deleting course.")
            }
        }
        if (loading) {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
        {courses.map((course) => (
          <div key={course._id} className="bg-white shadow-md rounded-lg p-4">
            ...
            {/* <p className="text-xl font-semibold mt-4 text-gray-800">created by {adminName}</p> */}
            {/* Course Description */}
